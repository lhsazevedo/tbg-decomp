...
matched_lines=$(grep -alE "SECTION\s+(P|C|D|B)" src/asm/matched/*.src | xargs cat | grep -avE ".IMPORT|.EXPORT" | wc -l)
decompiled_lines=$(grep -alE "SECTION\s+(P|C|D|B)" src/asm/decompiled/*.src | xargs cat | grep -avE ".IMPORT|.EXPORT" | wc -l)
remaining_lines=$(grep -alE "SECTION\s+(P|C|D|B)" src/asm/*.src | xargs cat | grep -avE ".IMPORT|.EXPORT" | wc -l)
